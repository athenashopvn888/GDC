...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://greendealcannabis.com/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | Green Deal Cannabis`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: any = {
    "@type": "Offer",
    url: `https://greendealcannabis.com/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://greendealcannabis.com${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
  ...
  return {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": item.category,
        "item": `https://greendealcannabis.com/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://greendealcannabis.com/item/${item.slug}`
      }
    ]
  };
}
