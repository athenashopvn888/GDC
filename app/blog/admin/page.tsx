...
export default function BlogAdmin() {
  ...
  function handleLogin(e: React.FormEvent) {
    e.preventDefault();
    if (username === "seowiz" && password === "greendealseo") {
  ...
  async function fetchPosts() {
    setLoading(true);
    try {
      const res = await fetch(`${APPS_SCRIPT_URL}?action=blog&store=GDC01&admin=1`);
    ...
    return (
      <div className={styles.loginPage}>
        <div className={styles.loginCard}>
          ...
          <p className={styles.loginSub}>Green Deal Cannabis</p>
