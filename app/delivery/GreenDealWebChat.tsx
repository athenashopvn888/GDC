...
export default function GreenDealWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Green Deal Cannabis chat could not be restored."));
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storeId: "GDC01", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, workflowVersion: "READY_V1", smsConsent }) }));
  ...
  return <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Green Deal Cannabis Web Chat">
    ...
    {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Green Deal Cannabis Web Chat">
      <header><div><strong>Green Deal Cannabis Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">&times;</button></header>
