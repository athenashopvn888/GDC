...
import GreenDealWebChat from "./GreenDealWebChat";
...
export default function DeliveryContent() {
  ...
  async function handleEmailSubmit(event: React.FormEvent) {
    ...
    try {
      await fetch(`https://script.google.com/macros/s/AKfycbx09_sDal1eMVF1r-hUck4e7oq_XBHEWhGvA79JuhZNQ6P4CdhCas0xE3FfexWQ3hq4/exec?action=delivery_email&email=${encodeURIComponent(email)}&store=GDC01`, { method: "GET", mode: "no-cors" });
    } finally {
  ...
  useEffect(() => {
    ...
    fetch("https://milestone-1-demo.vercel.app/api/catalog?store=GDC01", { signal: controller.signal })
  ...
  return <main className={styles.main}>
    ...
    <section className={`${styles.hero} ${styles.heroPlain}`}>
      <div><p>Green Deal Cannabis</p><h1>Delivery Menu</h1><span>Browse the shared product catalog. The store confirms current availability and delivery details before an order is accepted.</span></div>
    </section>
    <section className={styles.deliveryDetails} aria-label="Green Deal Cannabis delivery details"><strong>$60 PRODUCT MINIMUM</strong></section>
    ...
    <section className={styles.howToOrder} aria-labelledby="how-to-order-title"><div><p>HOW TO ORDER</p><h2 id="how-to-order-title">Order with the Green Deal Cannabis dispatcher</h2><span>LIVE ORDER connects you with the Green Deal Cannabis dispatcher.</span></div><ol><li><strong>Browse the delivery menu</strong><span>Note the product names and weights you want.</span></li><li><strong>Select LIVE ORDER</strong><span>Open Web Chat at the bottom-right and send your choices.</span></li><li><strong>Verify privately if you are new</strong><span>New customers complete the private selfie-with-ID step in Web Chat.</span></li><li><strong>Confirm with the dispatcher</strong><span>The dispatcher confirms availability, delivery details, and next steps.</span></li></ol></section>
      ...
      <div className={styles.catalog}>
        ...
        <div className={styles.grid}>{visible.map((product) => <article className={styles.card} key={product.publicProductId}>
          <button type="button" className={styles.imageButton} onClick={() => setSelected(product)} aria-label={`View details for ${product.name}`}>
            {product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on the Green Deal Cannabis delivery menu`} fill sizes="(max-width: 640px) 50vw, 280px" unoptimized /> : <span>Green Deal Cannabis</span>}
          </button>
    ...
    <section className={styles.updates} aria-labelledby="delivery-updates-title">
      <div><p>DELIVERY UPDATES</p><h2 id="delivery-updates-title">Get store delivery updates</h2><span>Enter your email to keep the existing Green Deal Cannabis delivery notification active.</span></div>
    ...
    <div className={styles.ctaSection}><p>Visit us in-store at <strong>1820 Jane St, York</strong>. We are open daily <strong>10am - 2am</strong>.</p></div>
