...
export const STAFF_BUCKET = "gdc-staff-photo-private";

export function staffDb() {
  const url = process.env.GDC_STAFF_SUPABASE_URL || "";
  const key = process.env.GDC_STAFF_SUPABASE_SERVICE_ROLE_KEY || "";
  if (!url || !key) throw new Error("Staff photo storage is not configured.");
  return createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false }, global: { headers: { "x-application-name": "gdc-staff-photo" } } });
}
