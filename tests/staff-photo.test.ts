...
test("daily PIN is deterministic and separated by store, day and version", () => {
  const pin = dailyPin(secret, "GDC01", "2026-07-23", 1);
  assert.match(pin, /^\d{4}$/);
  assert.equal(pin, dailyPin(secret, "GDC01", "2026-07-23", 1));
  assert.notEqual(pin, dailyPin(secret, "OTHER01", "2026-07-23", 1));
  assert.notEqual(pin, dailyPin(secret, "GDC01", "2026-07-22", 1));
  assert.notEqual(pin, dailyPin(secret, "GDC01", "2026-07-23", 2));
});
...
test("consecutive operational days never reuse the prior PIN", () => {
  ...
  for (let day = 0; day < 370; day += 1) {
    ...
    const pin = dailyPin(secret, "GDC01", key, 1);
...
test("previous PIN is rejected by default and optional grace is bounded", () => {
  process.env.GDC_STAFF_DAILY_PIN_SECRET = secret;
  process.env.GDC_STAFF_PIN_BOUNDARY_HOUR = "6";
  process.env.GDC_STAFF_PREVIOUS_PIN_GRACE_MINUTES = "0";
  const prior = dailyPin(secret, "GDC01", "2026-07-22", 1);
  assert.equal(verifyDailyPin(prior, 1, new Date("2026-07-23T10:00:01Z")), false);
  process.env.GDC_STAFF_PREVIOUS_PIN_GRACE_MINUTES = "15";
...
test("emergency version rotation invalidates the old PIN", () => {
  ...
  const oldPin = dailyPin(secret, "GDC01", "2026-07-23", 7);
...
test("consecutive emergency versions never reuse the prior PIN", () => {
  ...
  for (let version = 1; version <= 20; version += 1) {
    const pin = dailyPin(secret, "GDC01", "2026-07-23", version);
    if (previous) assert.notEqual(pin, previous);
    previous = pin;
  }
});
...
test("client source does not contain server secret names or PIN formula", () => {
  ...
  assert.equal(client.includes("GDC_STAFF_"), false);
...
test("deployment cleanup is scheduled and keeps separate cron authorization", () => {
  ...
  assert.match(route, /GDC_STAFF_CLEANUP_TOKEN/);
});
